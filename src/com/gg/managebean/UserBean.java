		...
		try {
			...
			if (ejb.getAllEntities(Users.class).stream().anyMatch(u -> u.getUserName().equals(user.getUserName()))) {
				FacesContext.getCurrentInstance().addMessage("messages",
						new FacesMessage(FacesMessage.SEVERITY_WARN, "Warning", "User name already exist."));
			} else if (ejb.getAllEntities(Users.class).stream().anyMatch(u -> u.getEmail().equals(user.getEmail()))) {
				FacesContext.getCurrentInstance().addMessage("messages",
						new FacesMessage(FacesMessage.SEVERITY_WARN, "Warning", "User email already exist."));
			} else if (ejb.getAllEntities(Users.class).stream().anyMatch(u -> u.getMobile().equals(user.getMobile()))) {
				FacesContext.getCurrentInstance().addMessage("messages",
						new FacesMessage(FacesMessage.SEVERITY_WARN, "Warning", "User mobile already exist."));
			} else {
				ejb.setData(getUser());
				FacesContext.getCurrentInstance().addMessage("messages",
			...
			FacesContext.getCurrentInstance().addMessage("messages",
					new FacesMessage(FacesMessage.SEVERITY_FATAL, "User creation failed", "Can not ctreate new User."));
		}
	}
}
